...
In \ours{}, independent data providers collectively form a \emph{database consortium} by publishing their block-reorganized data to a shared IPFS network. Each provider runs the block reorganizer locally to partition its tables into range-bounded shards and assemble them into an IPLD Merkle DAG with per-attribute range summaries (Section~\ref{sec:pushdown}). The resulting DAG root CID is then registered on the blockchain, making the provider's data discoverable and queryable by any authorized participant. Because every provider's data is organized with the same pushdown-friendly layout, the consortium as a whole becomes predicate-aware: the query engine can traverse any provider's Merkle DAG, prune irrelevant subtrees, and fetch only the matching shards---regardless of which peer originally uploaded the data. This design preserves data sovereignty, as each provider controls what data it publishes and under what access-control policies, while enabling efficient cross-provider analytical queries over the shared P2P storage layer.

\textbf{Blockchain.}
The blockchain serves as the coordination and trust layer of \ours{}. Following the model introduced by MtDB~\cite{hossain2025mtdb}, smart contracts maintain a global metadata registry that records each table's root CID, column schema, partitioning attributes, and access-control policies. When a data provider uploads a new table (or updates an existing one), the block reorganizer writes the new DAG to IPFS and submits a transaction that updates the on-chain root CID. The query engine consults the blockchain to discover available tables, retrieve the entry-point CID for Merkle DAG traversal, and verify that the requesting user is authorized to access the data. Because all metadata updates are recorded on an immutable ledger, the blockchain provides an auditable provenance trail: any participant can verify that a particular root CID was published by a specific provider at a specific time. The blockchain does not store any table data; it only records lightweight metadata, keeping on-chain costs minimal while providing the coordination guarantees needed for a multi-provider consortium.
