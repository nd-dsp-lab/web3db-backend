...
This section reviews the state of the art across five domains related to \ours{}. We begin with decentralized databases (\S\ref{sec:ipfs}) and then discuss the privacy and coordination models that motivate our use of a trusted execution environment (\S\ref{sec:tee}). We next examine data sharding techniques relevant to our block reorganizer (\S\ref{sec:sharding}), followed by predicate pushdown and query containment strategies that inform our hybrid query engine (\S\ref{sec:containment}). Finally, we discuss database caching approaches that shape our cache manager (\S\ref{sec:caching}).
...
The InterPlanetary File System (IPFS) provides content-addressed storage over a distributed hash table (DHT), allowing data to be stored and retrieved across peer nodes without a central server~\cite{benet2014ipfs,daniel2022ipfs,trautwein2022design}. Objects are identified by content identifiers (CIDs) and organized into Merkle directed acyclic graphs (DAGs)~\cite{benet2014ipfs}.

Several lines of work have explored building databases on this substrate. Early systems such as BigchainDB~\cite{mcconaghy2016bigchaindb} layered relational semantics directly over blockchain consensus, but this coupling imposed severe throughput limitations. Subsequent systems improved scalability by either distributing query execution across storage peers~\cite{yao2024minerva} or by using smart contracts to coordinate metadata and access control while keeping bulk data off-chain~\cite{hossain2025mtdb,mukherjee2025web3db}.

Despite these architectural advances, all existing designs treat IPFS as a generic object store: queries must retrieve entire remote partitions even when only a small fraction of the data is relevant. None of these systems support local caching or storage-level filtering, making network I/O the primary performance bottleneck. \ours{} addresses this gap by introducing both caching and predicate pushdown into the decentralized query path, thereby reducing unnecessary data transfer.
...
Trusted execution environments (TEEs) provide hardware-isolated memory regions that guarantee data confidentiality during computation~\cite{costan2016intel}. Several secure database systems exploit TEEs to protect analytical workloads inside enclaves~\cite{priebe2018enclavedb, zheng2017opaque}, while alternative approaches achieve privacy through computation over ciphertexts~\cite{popa2011cryptdb}. In the blockchain space, systems such as Enigma~\cite{zyskind2018enigma} and vChain~\cite{xu2019vchain} combine TEEs or cryptographic proofs with distributed ledgers, but their primary objective is computational privacy or verifiable correctness rather than query acceleration.

A common limitation of all these systems is that they assume fast local or cloud-attached storage, and consequently do not address the extreme latency penalties of peer-to-peer retrieval. \ours{} leverages the blockchain-coordinated metadata model of prior work~\cite{hossain2025mtdb} to manage access control, but fundamentally redesigns the off-chain storage and caching layers to minimize the retrieval latency that existing secure databases ignore.
...
Parallel and distributed databases rely extensively on range and hash partitioning to distribute workloads~\cite{dewitt1992parallel}. Modern cloud-native systems further refine this approach with dynamic shard splitting and online rebalancing~\cite{taft2020cockroachdb, bacon2017spanner, luo2022dynahash}. However, these techniques implicitly assume mutable storage: a shard can be split in place and its metadata updated atomically.

Content-addressed networks such as IPFS enforce immutability. Any modification produces a new object with a different CID, which invalidates the in-place update model. As a result, indexing strategies developed for decentralized networks have largely been confined to simple keyword lookups or data provenance tracking~\cite{arer2022efficient, rathee2019blockchain}. Although adaptive and learned index structures have shown promise in conventional mutable databases~\cite{kraska2018case}, they do not translate to immutable storage architectures. \ours{} bridges this gap by reorganizing data into range-bounded shards via extended KD-tree partitioning and assembling them into a custom IPLD Merkle DAG whose internal nodes carry per-attribute range summaries, enabling predicate-aware traversal at the storage level.
...
Predicate pushdown is a well-established optimization in which filters and aggregations are evaluated as close to the storage layer as possible, reducing the volume of data transferred to the query engine. Lakehouse architectures~\cite{armbrust2021lakehouse} and systems such as FlexPushdownDB~\cite{yang2021flexpushdowndb} routinely push predicates into the storage service or a near-storage compute layer. These approaches, however, rely on a storage layer that exposes a native query interface, a capability absent in content-addressed P2P networks.

A related challenge arises in semantic caching, where determining whether one cached query result subsumes another requires solving the query containment problem, which is computationally hard in general~\cite{chandra1977optimal}. Prior work has applied symbolic constraint solvers to verify query equivalences in an offline or static setting~\cite{he2024verieql, chu2017cosette}. \ours{} adapts both ideas to the decentralized context. We embed range metadata directly into the IPLD storage graph to achieve predicate pushdown without a native query interface, and it employs an SMT solver (Z3~\cite{de2008z3}) online to identify valid cached subsets dynamically during query execution.
...
Traditional database caches, including buffer pools, in-memory key-value stores, and distributed analytics caches, are designed for environments with fast local or cloud-attached storage~\cite{graefe2011modern, fitzpatrick2004distributed, redis2009, dageville2016snowflake, melnik2010dremel}. Multi-tier caching strategies further optimize performance in large-scale distributed settings~\cite{liu2019distcache, durner2021crystal, tang2024data, zhao2013hycache, wang2022metadata}. These designs, however, assume that cache misses incur modest latency penalties and therefore do not account for the extreme costs of peer-to-peer retrieval.

A more fundamental limitation is that standard result caches index entries by query string, which prevents them from exploiting semantic overlap between queries. Semantic caching~\cite{dar1996semantic} addresses this by annotating cached results with their defining predicates and evaluating containment relationships. Subsequent work has extended this idea with predicate indexing and intermediate result reuse~\cite{keller1996predicate, roy2000don}, while recent systems employ heuristic overlap representations for data warehouse workloads~\cite{you2025soc}. Because heuristic approaches sacrifice precision, \ours{} instead relies on a constraint solver~\cite{de2008z3} for rigorous containment validation.

Finally, cache eviction logic in the decentralized setting must be cost-aware. Classical eviction policies such as ARC~\cite{megiddo2003arc}, TinyLFU~\cite{einziger2017tinylfu}, LIRS~\cite{jiang2002lirs}, and SIEVE~\cite{zhang2024sieve} treat all cache entries as opaque blocks with uniform miss costs. When retrieval penalties vary by orders of magnitude, as they do in P2P networks, uniform-cost policies make suboptimal eviction decisions~\cite{podlipnig2003survey}. \ours{} introduces a utility-aware eviction policy that considers an entry's reuse potential and its retrieval cost, ensuring that high-value entries are retained when memory is constrained.
