...
@app.post("/upload-semantic/{table_name}")
...
async def query_semantic(request: SemanticQueryRequest):
    """
    ...
    """
    logger.info(f"POST /query-semantic - root_cid={request.root_cid}, query={request.query}")
    query_start = time.time()

    try:
        # Step 1: Parse predicates for pushdown
        predicates = parse_predicates(request.query, request.partition_attributes)
        ...
        traversal_result = traverser.traverse(request.root_cid, predicates)
        ...
        try:
            ...
            table_match = re.search(r'\bFROM\s+(\w+)', request.query, re.IGNORECASE)
            if table_match:
                original_table = table_match.group(1)
                exec_query = request.query.replace(original_table, "__semantic_data__")
            else:
                exec_query = request.query
