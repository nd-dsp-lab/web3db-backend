...
\title{\Large \bf \ours : Pushdown and Caching for Decentralized Databases}
...
\ours{} reduces network I/O for secure analytical query processing over decentralized databases. The literature that informs this problem spans \emph{decentralized databases}, \emph{secure query execution}, \emph{pushdown and data skipping}, and \emph{cache management}. Table~\ref{tab:comparison} summarizes how \ours{} relates to representative systems across these areas. We review previous work below.
...
\caption{Comparison of \ours{} with related work. \cmark\ indicates support; \xmark\ indicates no support.}
\label{tab:comparison}
\begin{tabular}{lccccc}
\toprule
\textbf{System} & \textbf{Decentralized} & \textbf{TEE} & \textbf{Pushdown} & \textbf{Cache} & \textbf{Hybrid Exec.} \\
\midrule
Minerva~\cite{yao2024minerva}              & \cmark & \xmark & \xmark & \xmark & \xmark \\
MtDB~\cite{hossain2025mtdb}                & \cmark & \cmark & \xmark & \xmark & \xmark \\
Web3DB~\cite{mukherjee2025web3db}          & \cmark & \xmark & \xmark & \xmark & \xmark \\
EnclaveDB~\cite{priebe2018enclavedb}       & \xmark & \cmark & \xmark & \xmark & \xmark \\
FlexPushdownDB~\cite{yang2021flexpushdowndb} & \xmark & \xmark & \cmark & \cmark & \cmark \\
\midrule
\textbf{\ours{}}                           & \textbf{\cmark} & \textbf{\cmark} & \textbf{\cmark} & \textbf{\cmark} & \textbf{\cmark} \\
\bottomrule
\end{tabular}
\end{table*}
 
%-------------------------------------------------------------------------------
\textbf{Decentralized databases.}
IPFS provides content-addressed storage built around content identifiers (CIDs), Merkle DAGs, and DHT-based provider discovery~\cite{benet2014ipfs,daniel2022ipfs,trautwein2022design}. These mechanisms decouple data naming from location and allow objects to be retrieved from multiple peers without a central server, making IPFS an attractive substrate for decentralized applications.
 
A growing line of work builds database functionality on top of this substrate. Early systems such as BigchainDB~\cite{mcconaghy2016bigchaindb} tightly coupled database semantics with blockchain coordination. More recent systems separate metadata management from bulk storage. Minerva~\cite{yao2024minerva} executes collaborative queries over IPFS and improves decentralized query processing through DHT-aware lookup optimization and a fat Merkle tree structure. MtDB~\cite{hossain2025mtdb} stores encrypted data off-chain while using blockchain-managed metadata for schemas, indexes, and access control. Web3DB~\cite{mukherjee2025web3db} similarly combines decentralized storage with relational abstractions and SQL-like operations to support user-controlled data ownership. Collectively, these systems establish the architectural pattern that \ours{} inherits: blockchain-coordinated metadata combined with off-chain decentralized storage. \ours{} extends this pattern by adding local cache and pushdown layer to the query path.
 
%-------------------------------------------------------------------------------
\textbf{Secure query execution.}
Protecting confidentiality during query execution has been studied through both cryptographic and hardware-assisted approaches. CryptDB~\cite{popa2011cryptdb} demonstrated that practical SQL processing can be supported over encrypted data by choosing encryption schemes according to operator requirements. A complementary direction uses trusted execution environments (TEEs), such as Intel SGX~\cite{costan2016intel}, to execute database operators inside protected memory regions. Opaque~\cite{zheng2017opaque} and EnclaveDB~\cite{priebe2018enclavedb} show that enclave-based execution can support expressive analytical workloads while reducing the trusted computing base compared with application-specific secure protocols.
 
In decentralized settings, secure computation has also been combined with distributed ledgers. Enigma explores privacy-preserving decentralized computation~\cite{zyskind2018enigma}, while vChain~\cite{xu2019vchain} studies authenticated query processing over blockchain-backed data. This literature motivates the secure execution and metadata-coordination layers of \ours{}: the enclave protects query processing and cached data, while the blockchain maintains trusted coordination metadata.
 
%-------------------------------------------------------------------------------
\textbf{Pushdown and data skipping.}
Classical parallel databases rely on partitioning and data placement to minimize inter-node traffic during query execution~\cite{dewitt1992parallel}. Modern distributed SQL systems refine these ideas with dynamic splitting, rebalancing, and geographically distributed placement~\cite{bacon2017spanner,taft2020cockroachdb,luo2022dynahash}. Learned and adaptive indexing schemes have further shown that physical layout can itself serve as a query-processing aid~\cite{kraska2018case}. However, these techniques assume mutable storage, where a shard can be split in place and its metadata updated atomically. In content-addressed networks such as IPFS, any modification produces a new object with a different CID, which invalidates the in-place update model. Indexing strategies developed for decentralized settings have largely been confined to keyword lookups or data provenance tracking~\cite{arer2022efficient, rathee2019blockchain}.
 
For analytical workloads over remote storage, predicate pushdown and data skipping have become standard mechanisms for reducing transferred bytes. Lakehouse systems exploit file- and block-level metadata to prune irrelevant data~\cite{armbrust2021lakehouse}, and FlexPushdownDB~\cite{yang2021flexpushdowndb} shows that pushdown can be combined with caching in a hybrid execution framework over disaggregated cloud storage. These systems rely on a storage layer that exposes a native query interface (e.g., S3~Select~\cite{hunt2018s3}), which content-addressed P2P networks do not provide. \ours{}'s block reorganizer encodes per-attribute range summaries directly in IPLD Merkle DAG~\cite{ipld2023spec} nodes to enable pushdown at the IPFS level.
 
%-------------------------------------------------------------------------------
\textbf{Cache management.}
Caching provides a complementary route to reducing remote accesses. Traditional database caches, buffer managers, and in-memory key-value stores are effective when misses can be serviced from local disks or datacenter-attached storage~\cite{graefe2011modern, fitzpatrick2004distributed, redis2009, dageville2016snowflake, melnik2010dremel}. Multi-tier and distributed caching strategies further optimize performance in large-scale settings~\cite{liu2019distcache, durner2021crystal, tang2024data, zhao2013hycache, wang2022metadata}. FlexPushdownDB\cite{yang2021flexpushdowndb} caches index entries by query string, which prevents them from exploiting semantic overlap between queries. Semantic caching~\cite{dar1996semantic} addresses this by annotating cached results with their defining predicates and evaluating containment relationships. Subsequent work extended this idea with predicate indexing and intermediate result reuse~\cite{keller1996predicate, roy2000don}, and recent systems employ heuristic overlap representations for data warehouse workloads~\cite{you2025soc}. \ours{}'s cache manager builds on the semantic caching model~\cite{dar1996semantic} and uses a constraint solver~\cite{de2008z3} for containment validation rather than heuristic matching.
 
% Reasoning about whether one query result can satisfy another is closely related to the query containment and equivalence problem, which is computationally difficult in general~\cite{chandra1977optimal}. Systems such as Cosette~\cite{chu2017cosette} and VeriEQL~\cite{he2024verieql} demonstrate that symbolic reasoning and SMT-based techniques can verify nontrivial SQL properties. This work informs the solver-based containment checking used in \ours{}.
 
For cache eviction, classical policies such as ARC~\cite{megiddo2003arc}, LIRS~\cite{jiang2002lirs}, TinyLFU~\cite{einziger2017tinylfu}, and SIEVE~\cite{zhang2024sieve} treat all cache entries as opaque blocks with uniform miss costs. Surveys of web and database caching have long observed that replacement decisions become cost-sensitive when miss penalties vary across objects~\cite{podlipnig2003survey}. FlexPushdownDB~\cite{yang2021flexpushdowndb} is the closest prior system to \ours{} in that it also combines caching and pushdown, but it targets a cloud disaggregated setting where miss penalties are modest and storage exposes a native query interface. \ours{} extends the SIEVE cache eviction algorithm~\cite{zhang2024sieve} with a utility score that balances re-fetch cost against semantic reuse potential, adapting it to the high-penalty P2P retrieval environment.
...
\emph{prefers the cache}: cached data is processed locally, and only the residual is pushed down, avoiding P2P latency entirely for the covered portion.  Second, the engine \emph{prefers pushdown over full fetch}: predicate-based DAG traversal retrieves far fewer shards than a full table scan, and in the decentralized setting---where each shard fetch costs 100--5\,000\,ms---even modest pruning ratios yield large latency savings.  Figure~\ref{fig:separable_plan} contrasts a
conventional plan that fetches every shard before processing with a separable plan that exploits both the cache and pushdown pruning.
...
We use the join query in Listing~1 to illustrate how the hybrid query engine handles multi-table queries (Figure~\ref{fig:separable_join}).
The database contains two relations, $P$ (Projects) and $T$ (Tasks), each stored as a Merkle DAG on IPFS with 8~leaf shards.
The in-enclave semantic cache holds partial results from prior queries: a cached result for $P$ satisfying \texttt{Budget}~$> x_c$ and a cached result for $T$ satisfying \texttt{Hours}~$> y_c$, where $x_c > x$ and $y_c > y$.

\textbf{Cache lookup.}
The cache manager invokes the Z3 containment checker for each relation independently.
For relation~$P$, the new predicate $P_n\!: \texttt{Budget} > x$ is tested against the cached predicate $P_c\!: \texttt{Budget} > x_c$.
Because $x < x_c$, the cached result covers only a subset of the needed rows: full containment does not hold.
The checker identifies partial overlap and computes the residual predicate $P_r\!: x < \texttt{Budget} \leq x_c$, which captures the uncovered portion.
The same procedure applies to relation~$T$, yielding a residual $P_r\!: y < \texttt{Hours} \leq y_c$.

\textbf{Cached-path execution.}
For each relation, the engine applies the original query predicate over the cached result inside the enclave.
The cached result for~$P$ (all rows with \texttt{Budget}~$> x_c$) is filtered to retain rows satisfying \texttt{Budget}~$> x$---which is trivially all cached rows, since $x_c > x$.
Similarly, all cached rows for~$T$ survive the filter \texttt{Hours}~$> y$.
These filtered subsets form the cached partial results $R_c$ for each relation.
No IPFS traffic is generated during this step.

\textbf{Pushdown-path execution.}
The residual predicates are pushed down through the respective Merkle DAGs.
For relation~$P$, the engine begins at the DAG root $r_P$ and checks each child's aggregate range against $x < \texttt{Budget} \leq x_c$.
One internal subtree is pruned entirely because its range falls outside the residual bounds; the surviving subtree expands to 4~matching leaf shards ($p_1$--$p_4$), while the pruned subtree's 4~leaves ($p_5$--$p_8$) are never fetched.
For relation~$T$, the same traversal under $y < \texttt{Hours} \leq y_c$ prunes more aggressively: only 2~of 8~shards ($t_1$--$t_2$) survive, with the remaining 6~pruned at the internal or leaf level.
The surviving shards are fetched concurrently, decrypted inside the enclave, and filtered under the residual predicate to produce the pushdown partial results $R_p$.

\textbf{Merge, join, and aggregation.}
For each relation, the merge operator concatenates the cached partial result $R_c$ and the pushdown partial result $R_p$ into a complete, deduplicated input relation.
The two merged relations are then joined on \texttt{P.ProjID~=~T.ProjID} using an in-enclave hash join, followed by the \texttt{GROUP BY P.Dept} aggregation that computes \texttt{SUM(T.Cost)}.
In our implementation, the merge and downstream operators are executed by DuckDB: the cached and fetched Arrow tables are registered as temporary tables and the original SQL query is re-executed over their union.
The final result is returned to the client, and the pushdown partial results are inserted into the cache under their respective residual predicates for future reuse.

% \textbf{Summary.}
Without any optimization, the engine would fetch all $8 + 8 = 16$ shards.
Pushdown alone reduces this to $4 + 2 = 6$ shards by pruning subtrees whose ranges fall outside the query predicates.
The hybrid plan fetches only the 6~shards needed for the residual predicates while serving the rest from the cache, and critically, future queries whose predicates overlap with either the original cached entries or the newly cached residuals will benefit from further cache hits.
The example demonstrates that the separable plan exploits both mechanisms simultaneously: caching eliminates IPFS fetches for the covered portion, and pushdown minimizes fetches for the uncovered portion.

\subsection{Access Control}
\label{sec:access_control}

In a database consortium, multiple data providers publish tables to a shared IPFS network, but each provider retains control over who may query its data and which rows are visible to each querier. \ours{} adopts MtDB's predicate-based access-control model~\cite{hossain2025mtdb}, in which fine-grained policies are stored on-chain and enforced at query time through SQL rewriting inside the TEE enclave.

\textbf{Policy model.}
An access-control policy is a triple $(t,\; w,\; P_{\mathit{ac}})$, where $t$ is a table identifier, $w$ is the Ethereum wallet address of the authorized querier, and $P_{\mathit{ac}}$ is a SQL predicate over the columns of $t$ that defines the accessible row subset. For example, a provider who publishes a \texttt{Patients} table may grant hospital~$A$ access only to its own patients by registering the policy $(\texttt{Patients},\; w_A,\; \texttt{HospitalID} = \text{`A'})$. Policies are recorded in the \texttt{TableRegistry} smart contract (Section~\ref{sec:impl_blockchain}) and are immutable once written; a provider revokes access by submitting a new transaction that removes the policy entry.

\textbf{Query rewriting.}
When a query $Q$ referencing table $t$ arrives from wallet $w$, the query engine retrieves all policies $(t, w, P_{\mathit{ac}}^{(i)})$ associated with $w$ from the smart contract. If no matching policy exists, the query is rejected. Otherwise, the engine rewrites $Q$ by injecting an additional conjunctive filter:
\begin{equation}
    Q' \;=\; Q \;\wedge\; \Bigl(\bigvee_{i} P_{\mathit{ac}}^{(i)}\Bigr)
    \label{eq:rewrite}
\end{equation}
The disjunction combines all applicable policies for $w$, so a querier who holds multiple grants (e.g., from different data providers) sees the union of all authorized rows. The rewritten query $Q'$ then follows the normal execution path: the cache manager checks containment against $Q'$'s predicates, and the pushdown engine traverses the Merkle DAG under $Q'$'s combined predicate, including the access-control filter. Because the rewriting occurs inside the enclave before any data is accessed, unauthorized rows are never decrypted or returned.

\textbf{Integration with caching and pushdown.}
The injected access-control predicate participates in both the semantic cache and the DAG traversal without special handling. When the cache manager performs Z3 containment checking (Section~\ref{sec:z3}), the access-control conjunct is part of the query predicate, so a cached result produced under one policy is never served to a querier with a different policy. Similarly, during Merkle DAG traversal (Section~\ref{sec:dag_traversal}), the access-control predicate further prunes shards whose bounding boxes fall outside the authorized range, reducing network I/O beyond what the user's analytic predicates alone would achieve.
...
All core modules run inside the SGX enclave via the Gramine library OS~\cite{tsai2017graphene,kuvaiskii2022computation}, which allows unmodified Linux applications to execute in an enclave with minimal porting effort.
...
We evaluate \ours{} using the TPC-H decision-support benchmark~\cite{tpch}, a widely adopted standard for OLAP systems~\cite{yang2021flexpushdowndb}. The unit of analysis is a single analytical query, and the target population is the space of OLAP workloads executed over \ours{}. We generate datasets at scale factors SF\,=\,1, 10, and 100 (approximately 1\,GB, 10\,GB, and 100\,GB) with the standard \texttt{dbgen} tool and sample the query space by constructing five independent, randomized streams of 100 TPC-H queries each. We compare four system configurations: \ours{} (hybrid), caching-only, pushdown-only, and a no-optimization baseline. The independent variables are the system configuration, TPC-H scale factor, and cache capacity as a fraction of the dataset size. Data is collected through automated instrumentation of the \ours{} query engine, which logs per-query measurements at each stage of the execution pipeline. For every query, we record end-to-end latency (from submission to result delivery), the number of shard fetches, total bytes transferred from IPFS, cache disposition (full hit, partial hit, or miss), Merkle DAG pruning ratio, and Z3 containment-check time. All results are averaged over the five streams to reduce variance.

Several confounding factors may influence the results. IPFS retrieval latency varies with DHT state and peer availability; we control for this by pinning all data on consortium peers before each experiment. SGX overhead from Enclave Page Cache (EPC) paging and memory encryption can inflate in-enclave processing time; we report it separately from network time to isolate its effect. During cold start, the semantic cache contains too few entries for containment checking to be effective; we therefore distinguish cold-start from steady-state results. Finally, TPC-H's uniformly distributed synthetic data may not capture real-world skew; we acknowledge this limitation and leave evaluation on production consortium workloads to future work.
\section{Conclusion}
