...
# Define colors and styles for lines
colors = {
...
line_styles = {
    'with_vanilla': '-',      # Solid line
    'with_sgx': '--',         # Dashed line
    'without_vanilla': '-.',  # Dash-dot line
    'without_sgx': ':'        # Dotted line
}

# Plot the lines with markers
ax.plot(x, with_index_nosgx, marker='o', linewidth=2.5, markersize=8, 
        label='With Index (Vanilla)', color=colors['green'], 
        linestyle=line_styles['with_vanilla'])

ax.plot(x, with_index_sgx, marker='s', linewidth=2.5, markersize=8, 
        label='With Index (SGX)', color=colors['blue'], 
        linestyle=line_styles['with_sgx'])

ax.plot(x, without_index_nosgx, marker='^', linewidth=2.5, markersize=8, 
        label='Without Index (Vanilla)', color=colors['orange'], 
        linestyle=line_styles['without_vanilla'])

ax.plot(x, without_index_sgx, marker='D', linewidth=2.5, markersize=8, 
        label='Without Index (SGX)', color=colors['red'], 
        linestyle=line_styles['without_sgx'])
...
plt.savefig("mtdb_query_execution_time_line.pdf", format='pdf', dpi=300, bbox_inches='tight')
plt.show()
